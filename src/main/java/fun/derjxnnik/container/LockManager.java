...
public class LockManager {
   ...
   private final Map<String, Map<String, LockData>> locks = new HashMap<>();
   ...
   private LockData getLockData(Location loc) {
      return this.locks.getOrDefault(this.worldKey(loc), Collections.emptyMap()).get(this.key(loc));
   }

   public boolean isLocked(Location loc) {
      return this.locks.getOrDefault(this.worldKey(loc), Collections.emptyMap()).containsKey(this.key(loc));
   }
   ...
   public List<String> getTrustedDisplay(Location loc) {
      LockData data = this.getLockData(loc);
   ...
   public synchronized void saveLocks() {
      try {
         ...
         for(String world : this.locks.keySet()) {
            for(Map.Entry<String, LockData> entry : this.locks.get(world).entrySet()) {
               LockData data = entry.getValue();
               String path = "locks." + world + "." + entry.getKey();
   ...
   private boolean doLock(Location loc, String owner) {
      this.locks.computeIfAbsent(this.worldKey(loc), (w) -> new HashMap<>()).put(this.key(loc), new LockData(owner, new HashSet<>()));
      return true;
   }

   private boolean doUnlock(Location loc, String actor) {
      Map<String, LockData> map = this.locks.get(this.worldKey(loc));
   ...
   public boolean removeLockCompletely(Location loc) {
      ...
      Map<String, LockData> map = this.locks.get(world);
      if (map == null) {
         return false;
      } else {
   ...
   public void lockMany(List<Location> locs, Player player) {
   ...
   public void unlockMany(List<Location> locs, Player player) {
   ...
   public void trustMany(List<Location> locs, Player owner, String target) {
   ...
   public void untrustMany(List<Location> locs, Player owner, String target) {
      List<Location> locked = locs.stream().filter(this::isLocked).toList();
      if (locked.isEmpty()) {
         owner.sendMessage(Colors.PREFIX + Colors.RED + "This container is not locked.");
      } else {
   ...
   public void transferMany(List<Location> locs, Player owner, String target) {
      List<Location> locked = locs.stream().filter(this::isLocked).toList();
      if (locked.isEmpty()) {
         owner.sendMessage(Colors.PREFIX + Colors.RED + "This container is not locked.");
      } else {
   ...
   private static class LockData {
      ...
      LockData(String owner, Set<String> trusted) {
         this.owner = owner;
